...
确定性整数中文读写和精确分单位大写金额。本地候选版 0.2.0，供比较和代码审查；尚未作为完整竞赛作品提交。
...
实现目标：正负整数、万亿分组、常用/财务数字、精确角分。

未承诺：任意精度、口语歧义、外币规则、浮点金额输入。

## 来源与实现方式

规格/算法参考：https://zh.wikipedia.org/wiki/中文数字。
...
补口语数字宽松解析、更多计数单位与可配置财务书写格式。当前只接受规范中文整数，不接收“一万二”等歧义表达；金额采用精确整数分，不做浮点四舍五入。
...
打开 http://127.0.0.1:8780/web/ 。修改和测试源码需安装 MoonBit 与 Node.js，再运行 `./verify.ps1`。本机尚未将 MoonBit 加入 PATH 时，可传入 `-MoonPath`。独立包不捆绑编译器。
...
仅接受本库规范金额格式；非规范、方言或自然语言金额不自动猜测。
